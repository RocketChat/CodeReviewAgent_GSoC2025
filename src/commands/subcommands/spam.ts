...
import { IModify, IRead, IPersistence, IPersistenceRead } from '@rocket.chat/apps-engine/definition/accessors';
...
import { getSectionBlock, getActionsBlock, getButton, getDividerBlock, getContextBlock } from '../../helpers/blockBuilder';
...
export async function handleSpamCommand(
    ...
    switch (action) {
        ...
        case undefined: // Default action
            await handleListSpamReviews(app, read, modify, user, room, persistence, persistenceRead);
            break;
        
        case 'approve':
            await handleSpamAction(app, read, modify, user, room, persistence, persistenceRead, args[1], 'approve');
            break;
            
        case 'reject':
            await handleSpamAction(app, read, modify, user, room, persistence, persistenceRead, args[1], 'reject');
            break;
            
        case 'ignore':
            await handleSpamAction(app, read, modify, user, room, persistence, persistenceRead, args[1], 'ignore');
            break;
...
async function handleListSpamReviews(
    ...
    try {
        ...
        const pendingReviews = await spamService.getPendingSpamReviews(persistenceRead);
...
async function handleSpamAction(
    ...
    try {
        ...
        const spamItem = await SpamReviewPersistence.getSpamReviewItem(prId, persistenceRead);
        ...
        if (spamItem.status !== 'pending') {
            await sendNotification({
                ...
                user: user,
                room: room,
                message: `❌ **PR already reviewed**\n\nPR #${spamItem.prNumber} was already ${spamItem.status} by ${spamItem.reviewedBy || 'unknown'}`
            });
            return;
        }

        // Perform the action
        await spamService.reviewSpamItem(prId, action, user.id, persistence, persistenceRead);
        ...
        const actionDescriptions = {
            approve: 'The PR has been marked as legitimate and will proceed to reviewer assignment.',
            reject: 'The PR has been confirmed as spam and will be flagged accordingly.',
            ignore: 'The PR has been marked to ignore - no further action will be taken.'
        };
...
**View PR:** https://github.com/${spamItem.repoName}/pull/${spamItem.prNumber}`
        ...
        app.getLogger().info(`Admin ${user.username} ${action}ed spam review for PR ${prId}`);

    } catch (error) {
        app.getLogger().error(`Failed to ${action} spam review ${prId}: ${error.message}`);
        await sendNotification({
            modify: modify,
            user: user,
            room: room,
...
function buildSpamListMessage(pendingReviews: SpamReviewItem[]): string {
    ...
    const header = `🚨 **${count} Pending Spam Review${count === 1 ? '' : 's'}**\n\n`;
    
    const reviews = pendingReviews.slice(0, 10).map((item, index) => {
        const flags = item.flags.length > 0 ? ` | Flags: ${item.flags.join(', ')}` : '';
...
function buildSpamListBlocks(pendingReviews: SpamReviewItem[]): LayoutBlock[] {
    const blocks: LayoutBlock[] = [
        getSectionBlock(`🚨 **${pendingReviews.length} Pending Spam Review${pendingReviews.length === 1 ? '' : 's'}**`)
    ];
    ...
    if (pendingReviews.length > 5) {
        blocks.push(
            getContextBlock(`*Showing first 5 of ${pendingReviews.length} pending reviews. Use \`/code-review-agent spam list\` to see all.*`)
        );
    }
