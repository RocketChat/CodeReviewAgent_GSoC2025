...
// Import persistence
import { UserMappingPersistence, UserMapping } from "./src/persistence/UserMappingPersistence";
...
${quickReminder}Need help? Use \`/code-review-agent help\` for available commands.`;
    ...
    switch (id) {
        case AppSettingsEnum.REPOSITORIES_LIST_ID: {
            ...
            if (repositories.length === 0) {
                this.getLogger().warn('Invalid repository list provided, keeping old value');
                return context.oldSetting;
            }

            // Note: Codeowners sync will be handled by the scheduled job
            this.getLogger().info('Repository list updated, codeowners will be synced on next scheduled run');
            break;
        }

        case AppSettingsEnum.ORG_ADMIN_PAT_TOKEN_ID: {
            if (!ValidationHelper.isValidGitHubToken(value)) {
                this.getLogger().warn('Invalid GitHub token format, keeping old value');
                return context.oldSetting;
            }
            break;
        }
        ...
        case AppSettingsEnum.GEMINI_API_KEY_ID: {
            if (!ValidationHelper.isValidApiKey(value)) {
                this.getLogger().warn('Invalid Gemini API key, keeping old value');
                return context.oldSetting;
            }
            break;
        }
    ...
    public async onSettingUpdated(
        ...
        switch (setting.id) {
            case AppSettingsEnum.REPOSITORIES_LIST_ID:
                // Immediate codeowners sync for better UX (don't wait for scheduled job)
                this.getLogger().info('Repository list updated, triggering immediate codeowners sync');
                try {
                    ...
                    //         this.getLogger().info(`Updated CODEOWNERS for ${repoName}`);
                    //     }
                    // }
                } catch (error) {
                    this.getLogger().error(`Failed to sync codeowners after settings update: ${error.message}`);
                }
                break;
            ...
            case AppSettingsEnum.GEMINI_MODEL_ID:
                // Clear Gemini-dependent services
                this.geminiService = undefined;
                this.spamDetectionService = undefined;
                this.reviewerMatchingService = undefined;
                this.getLogger().info(`Gemini setting '${setting.id}' updated, cleared dependent services`);
                break;

            case AppSettingsEnum.ORG_ADMIN_PAT_TOKEN_ID:
            case AppSettingsEnum.OWNER_NAME_ID:
                // Clear GitHub-dependent services
                this.githubService = undefined;
                this.prService = undefined;
                this.codeownersService = undefined;
                this.reviewerMatchingService = undefined;
                this.getLogger().info(`GitHub setting '${setting.id}' updated, cleared dependent services`);
                break;
        }
    }
    ...
    public async runPRProcessingPipeline(
        ...
        try {
            ...
            const newPRs = await prService.fetchAndStoreNewPRs(persistence, persistenceRead);
            ...
            for (const prData of newPRs) {
                try {
                    ...
                    this.getLogger().error(`Failed to process PR ${prData.id}: ${error.message}`);
                    // Continue with other PRs even if one fails
                }
            }

            this.getLogger().info(`PR processing pipeline completed for ${newPRs.length} PRs`);
    ...
    private async processSinglePR(
        ...
        try {
            ...
            const spamResult = await spamService.analyzePR(prData, repoName, persistence);

            if (spamResult.isSpam) {
                // Update PR status and notify admins
                await prService.updatePRStatus(prId, 'spam_detected', persistence, persistenceRead, {
                ...
                await notificationService.notifyAdminsOfSpamDetection(
                    prData, repoName, spamResult.spamScore, spamResult.reasoning,
                    read, modify, persistence
                );

                this.getLogger().warn(`PR ${prId} flagged as spam (score: ${spamResult.spamScore})`);
            ...
            const reviewers = await reviewerService.findReviewers(persistenceRead, prData, repoName);

            if (reviewers.length === 0) {
                await prService.updatePRStatus(prId, 'failed', persistence, persistenceRead, {
            ...
            await prService.updatePRStatus(prId, 'reviewers_assigned', persistence, persistenceRead, {
            ...
            const notificationResult = await notificationService.notifyReviewers(
                prData, reviewers, repoName, read, modify, persistence
            );

            // Step 5: Notify admins of successful processing
            await notificationService.notifyAdminsOfProcessing(
                prData, repoName, reviewers, notificationResult,
                read, modify, persistence
            );

            // Final status update
            const finalStatus = notificationResult.successful > 0 ? 'clean' : 'failed';
            await prService.updatePRStatus(prId, finalStatus, persistence, persistenceRead);
     ...
     */
    public async validateSettings(): Promise<{ valid: boolean; errors: string[] }> {
        const errors: string[] = [];
        
        try {
            const settingsReader = this.getAccessors().environmentReader.getSettings();
            // Validate GitHub settings
            const owner = await settingsReader.getValueById(AppSettingsEnum.OWNER_NAME_ID);
            if (!owner || !ValidationHelper.isValidGitHubUsername(owner)) {
                errors.push('Invalid GitHub owner/organization name');
            }
            
            const token = await settingsReader.getValueById(AppSettingsEnum.ORG_ADMIN_PAT_TOKEN_ID);
            if (!token || !ValidationHelper.isValidGitHubToken(token)) {
                errors.push('Invalid GitHub Personal Access Token');
            }
            
            const repositoriesRaw = await settingsReader.getValueById(AppSettingsEnum.REPOSITORIES_LIST_ID);
            if (!repositoriesRaw) {
                errors.push('Repository list is required');
            } else {
                const repositories = ValidationHelper.sanitizeRepositoryList(repositoriesRaw);
                if (repositories.length === 0) {
            ...
            const apiKey = await settingsReader.getValueById(AppSettingsEnum.GEMINI_API_KEY_ID);
            if (!apiKey || !ValidationHelper.isValidApiKey(apiKey)) {
                errors.push('Invalid Gemini API key');
            }
            
            const model = await settingsReader.getValueById(AppSettingsEnum.GEMINI_MODEL_ID);
            if (!model) {
